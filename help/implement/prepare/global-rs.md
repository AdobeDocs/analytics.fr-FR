...
* **Prise en charge des analyses sur plusieurs périphériques :** CDA requiert une suite de rapports qui collecte des données à plusieurs emplacements, tels que votre site Web et votre application mobile. Des appareils distincts peuvent assembler des données si leur mise en œuvre est correcte. Voir [Analyses sur plusieurs périphériques](../../components/cda/overview.md) dans le guide de l’utilisateur Composants pour plus d’informations.
