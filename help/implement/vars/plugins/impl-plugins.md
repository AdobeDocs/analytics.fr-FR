...
2. Coller le code du plug-in à l’aide de l’éditeur de code personnalisé de 
...
1. Instanciez d’abord l’objet de suivi Analytics (avec [`s_gi`](../functions/s-gi.md)).
   * Votre site compatible avec les balises instancie automatiquement l’objet de suivi au chargement d’Adobe Analytics.
...
2. Deuxièmement, intégrez le code du plug-in.
   ...
   * Si votre mise en oeuvre n’utilise pas de balises dans Adobe Experience Platform, vous pouvez coller le code du plug-in dans `AppMeasurement.js` n’importe où après avoir appelé l’objet de suivi.
3. Troisièmement, appelez le plug-in.
   * Toutes les mises en oeuvre, que ce soit à l’intérieur ou à l’extérieur d’un site compatible avec les balises, utilisent JavaScript pour appeler des modules externes. Appelez le plug-in en respectant le format indiqué sur la page de ce plug-in.
