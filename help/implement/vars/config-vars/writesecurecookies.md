...
La `writeSecureCookies` variable permet à AppMeasurement de définir des cookies [](https://en.wikipedia.org/wiki/Secure_cookie) sécurisés pour Analytics. Ce paramètre s’applique aux cookies d’ID de définis par AppMeasurement et aux cookies que vous définissez à l’aide de la `Util.CookieWrite()` méthode. Il nécessite AppMeasurement 2.18.0 ou une version ultérieure.
