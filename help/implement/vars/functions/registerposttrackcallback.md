...
> [!NOTE] Le timing et l&#39;ordre des fonctions déclenchées entre [`registerPreTrackCallback`](registerpretrackcallback.md) et `registerPostTrackCallback` ne sont pas garantis. Evitez les dépendances entre ces deux fonctions.
...
D’autres arguments peuvent être inclus dans la `s.registerPostTrackCallback` fonction, qui peut être utilisée dans la fonction imbriquée :
