---
description: Data Warehouse vous permet de demander des rapports qui présentent un niveau avancé de relations entre les données brutes, en fonction de vos questions.
keywords: ftp;sftp
title: Rapports distribués sur l’Data Warehouse
feature: FTP Export
exl-id: d49217a4-33ab-49f8-9207-c5d31e8f798d
TQID: https://experienceleague.adobe.com/ZgQ9y-yl5OLMfy4uP4EL-47i60hjbDwaojRVn1HXnys
product_v2: id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2: id: b069d60e-95f3-44d6-95a8-ddc862a4bc38
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
source-git-commit: ff16e07c7a2b75e9c6cc09e8255a7ea7e4c6f0c8
workflow-type: tm+mt
source-wordcount: 180
ht-degree: 52%

---

# Rapports distribués sur l’Data Warehouse

>[!NOTE]
>
>Les informations suivantes concernent les types de destinations FTP et SFTP. Les FTP et SFTP sont des types de destination hérités. Lors de la configuration d’un rapport Data Warehouse, vous devez utiliser un type de destination cloud, qui est plus sécurisé. Pour plus d’informations sur la configuration des types de destination cloud pour un rapport Data Warehouse, voir [Création d’une requête Data Warehouse](/help/export/data-warehouse/create-request/t-dw-create-request.md).

Data Warehouse vous permet de demander des rapports qui présentent un niveau avancé de relations entre les données brutes, en fonction de vos questions.

Dans ce cas, la distribution FTP est disponible dans le cadre de la configuration des [!UICONTROL options de remise avancées]. Pour utiliser le protocole FTP, vous avez besoin d’un compte FTP interne existant ou d’un compte FTP Adobe. Adobe ne configure pas automatiquement le compte FTP en votre nom par l’intermédiaire de l’interface de l’entrepôt de données. La diffusion FTP doit être immédiate, selon la taille du fichier et le nombre de rapports traités.

Pour plus d’informations sur les limites FTP et la conservation des données, voir [Limites FTP et conservation des données](/help/export/ftp-and-sftp/ftp-limits.md).
