...
# Vue d’ensemble du contenu du flux de données
...
Certains flux sont configurés en vue de recevoir un fichier `.fin` plutôt qu’un manifeste `.txt`. L’élément `.fin` indique que le chargement est terminé, mais que les métadonnées qu’il contient sont dans un format plus ancien.
...
* **`color_depth.tsv`** :mappe l’identifiant de profondeur de couleur (la colonne de flux `color`) à la profondeur de couleur.
* **`connection_type.tsv`** : mappe l’identifiant de type de connexion (la colonne de flux `connection_type`) au type de connexion.
* **`country.tsv`** : associe l’identifiant de pays (la colonne de flux `country`) au nom du pays.
* **`javascript_version.tsv`** : mappe l’identifiant de version JavaScript (la colonne de flux `javascript`) à la version JavaScript.
* **`languages.tsv`** : mappe l’identifiant de langue (la colonne de flux `language`) à la langue.
...
* **`plugins.tsv`** : mappe les identifiants des plug-ins (la colonne de flux `plugin`) à chaque nom de plug-in correspondant.
* **`resolution.tsv`** : mappe l’identifiant de résolution (la colonne de flux `resolution`) à la résolution du moniteur.
* **`referrer_type.tsv`** : mappe l’identifiant de type référent (la colonne de flux `ref_type`) au type de référent.
...
Lorsque les données sont collectées pour une heure, vous recevez un ou plusieurs fichiers de données compressés et un fichier de manifeste. Le fichiers de données s’intitulent :

`[index]-[rsid]_[YYYYmmdd]-[HHMMSS].[format_suffix].[compression_suffix]`

Après extraction, chaque fichier de données contient un fichier `[index]-[rsid]_[YYYYmmdd]-[HHMMSS].[format_suffix]` unique qui contient environ 2 Go de données non compressées, ainsi que des fichiers de recherche pour toutes les colonnes demandées.
