...
Dans Adobe Analytics, il existe deux niveaux d’organisation des silos de données :
...
Les suites de rapports virtuelles sont également disponibles dans Adobe Analytics, qui fournit une vue filtrée d’une suite de rapports sans modifier la collecte de données ni les données historiques. Vous pouvez, par exemple, utiliser une suite de rapports virtuelle pour éliminer un trafic de bots qui ne l’a pas encore été ou pour exclure les recettes frauduleuses/aberrantes. Voir [Suites de rapports virtuelles](/help/components/vrs/vrs-about.md) dans le guide d’utilisation des composants pour en savoir plus.
