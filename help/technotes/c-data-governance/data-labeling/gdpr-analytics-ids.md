...
* Certains identifiants peuvent correspondre à plusieurs personnes et vous ne souhaitez pas risquer de renvoyer des informations sur une personne à une autre possédant le même identifiant. Par exemple, même si vous pouvez vérifier que le nom de la personne est John Smith, vous ne voulez peut-être pas renvoyer l’intégralité des données relatives à tous les John Smith de votre système.
...
* Les étiquettes DEL- permettent d’[anonymiser](/help/technotes/c-data-governance/data-labeling/gdpr-labels.md#data-governance-labels) ces variables (l’ID sera remplacé par une chaîne aléatoire accompagnée du terme « Data Privacy »). La même valeur rendue anonyme remplacera toutes les instances de la valeur d’origine dans tous les accès identifiés par un ID utilisé dans la demande. Si la valeur d’origine dans ce champ était l’un de ces ID, alors les mesures du rapport ne changeraient pas.
...
* Si vous ne spécifiez pas l’utilisation de l’extension d’ID, mais que vous utiliserez un mélange d’identifiants d’appareil et de personne pour différentes demandes, vous pouvez spécifier les étiquettes DEL-DEVICE et DEL-PERSON pour les variables qui doivent être supprimées lors de l’utilisation de l’un des types d’identifiants.
