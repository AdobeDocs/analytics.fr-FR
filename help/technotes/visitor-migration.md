...
>[!NOTE]
>
>Si vous avez déjà mis en œuvre le service d’identification des visiteurs Experience Cloud, la période de grâce ne vous concerne pas et ne doit pas être activée.

La migration de visiteurs est un processus par lequel le ou les cookies d’identifiant visiteur sont migrés d’un domaine à un autre.
...
<table id="table_7B2535FC3E264216A299686415C6B21C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>Utilisez un moniteur de paquets <a href="../implement/validate/packet-monitor.md"></a> pour vérifier que lorsque vous accédez à votre site pour la première fois, ou après avoir effacé les cookies, deux codes d’état HTTP 302 (redirection) s’affichent avant le code d’état HTTP 200 (OK). S’il manque l’une de ces redirections, contactez immédiatement l’assistance clientèle afin de vous assurer que la migration est correctement configurée. </p> </td> 
  </tr> 
  <tr> 
...
| Tâche | Description |
|--- |--- |
| Pour commencer : contactez l’assistance clientèle avec le ou les domaines à migrer et la période de migration que vous souhaitez activer (30, 60 ou 90 jours). Veillez à inclure les domaines non sécurisés et sécurisés. | Créez une liste avec la syntaxe exacte des domaines vers lesquels vous souhaitez effectuer la migration et à partir desquels effectuer la migration.<ul><li>example.112.2o7.net > metrics.example.com</li><li>example.102.112.2o7.net > smetrics.example.com</li></ul>Les noms d’hôte de migration sont configurés sur le serveur de collecte de données Adobe. Une fois le changement effectué, l’assistance clientèle vous en informera afin que vous puissiez planifier l’étape suivante. |
| 6 heures ou plus après le changement de configuration : mettez à jour les variables `s.trackingServer` et `s.trackingServerSecure` dans votre code Analytics JavaScript pour utiliser les nouveaux serveurs de collecte de données. | Après avoir effectué cette modification, utilisez [Experience Cloud Debugger](https://experienceleague.adobe.com/docs/debugger/using/experience-cloud-debugger.html?lang=fr) pour vérifier que la demande d’image Analytics est envoyée au serveur de collecte de données mis à jour. |
| Immédiatement après la mise à jour de votre code Analytics : testez votre site pour vérifier que la redirection vers le domaine de collecte de données précédent est en cours. | Utilisez un [moniteur de paquets](../implement/validate/packet-monitor.md) pour vérifier que lorsque vous accédez à votre site pour la première fois, ou après avoir effacé les cookies, deux codes d’état HTTP 302 (redirection) s’affichent avant le code d’état HTTP 200 (OK). S’il manque l’une de ces redirections, contactez immédiatement l’assistance clientèle afin de vous assurer que la migration est correctement configurée. |
| Pendant toute la période de migration : laissez l’enregistrement DNS du nom d’hôte précédent actif. | Le nom d’hôte précédent doit être résolu par DNS, sans quoi la migration des cookies n’aura pas lieu. |