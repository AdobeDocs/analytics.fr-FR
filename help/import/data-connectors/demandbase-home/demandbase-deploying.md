...
1. Déployez le code à l’aide de l’une des méthodes suivantes :
   ...
   * Vous pouvez également diffuser le code à la ressource d’organisation responsable de la mise à jour du code de déploiement Adobe Analytics.

>[!IMPORTANT]
>
>Vérifiez que vous testez le déploiement de cette intégration dans un environnement de développement/test avant de le déployer dans un environnement de production.

## Modification du code d’intégration{#modifying-the-integration-code}

Dans la plupart des cas, vous n’avez pas besoin d’apporter des modifications au code d’intégration généré par l’assistant Connector de données.
...
<table id="table_5405A73CEFD44466B3C39559F4A037C9"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> Cette chaîne est précédée de chaque nom de dimension Demandbase avant d’être envoyée à Adobe Target. Par exemple, si ce paramètre a la valeur "db_", la dimension "industrie" sera envoyée à Adobe Target sous la forme "db_industrie". </td> 
  </tr> 
  <tr> 
...
1. Copiez et collez le contenu de ce fichier dans votre [!DNL AppMeasurement.js] fichier principal.

   >[!NOTE]
   >
   >Collez-le juste avant le commentaire DO NOT ALTER ANYTHING BELOW This LINE dans le fichier.

### Pour le code hérité (code H) {#section-bba8ad8c715e4f97883e7de3269f681a}

1. Téléchargez le module Integrate à partir de la zone "Resources" de l’interface utilisateur des connecteurs de données (sous l’onglet Support).

   ![](assets/h_code.png)

1. Copiez et collez le contenu de ce fichier dans votre [!DNL s_code] fichier.
...
Vérifiez que l’intégration capture correctement les données en vérifiant le suivi en direct et la création de rapports.
...
**Les rapports et analyses (anciennement SiteCatalyst) incluent les deux variables de données contextuelles Demandbase :**
...
Consultez vos rapports Demandbase dans Adobe Analytics à l’aide du tableau de bord qui a été automatiquement créé pour vous à l’aide de l’assistant d’intégration Adobe (Etape 7).
