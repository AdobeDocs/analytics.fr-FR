...
1. Cliquez sur **[!UICONTROL Admin]** > **[!UICONTROL Data Connectors]**.

   ![](assets/data_connectors.png)

1. Recherchez **[!UICONTROL DoubleClick DFA]**, puis cliquez sur **[!UICONTROL Add New]**.

   ![Résultat de l’étape](assets/wizard-01.png)

   On each page of the Integration Wizard, provide the required information, then click **[!UICONTROL Next]**. Le tableau suivant décrit les informations dont vous avez besoin pour terminer l’intégration via l’assistant.

<table id="table_8F6F7F304C36431DA5FD6E5D54F60FC0"> 
 <thead> 
  <tr> 
   <th colname="col1" class="entry"> Page Assistant # </th> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> 1 </td> 
   <td colname="col2"> Adresse électronique d’intégration </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> Variable de  publicitaire </td> 
   <td colname="col3"> <p>eVar Analytics qui reçoit les données d’affichage publicitaire DFA. La variable -publicitaire vous permet de voir comment les -publicitaires affectent les  publicitaires sur votre site. </p> <p>La fonctionnalité Data Connectors ajoute les mêmes classifications liées à DFA pour cette eVar que pour la variable publicitaire DFA (voir ci-dessus). </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 5 </td> 
   <td colname="col2"> Durée depuis le dernier (variable d’intervalle de temps  publicitaire) </td> 
   <td colname="col3"> eVar Analytics qui reçoit les données de durée depuis le dernier affichage DFA. Le  Durée depuis le dernier indique la durée écoulée depuis le dernier  publicitaire publicitaire. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 5 </td> 
   <td colname="col2">  publicitaires des </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> Echec du  DFA </td> 
   <td colname="col3"> (Facultatif) eVar Analytics qui reçoit les codes de message d’erreur de requête DFA signalés. Les codes de message DFA possibles sont les suivants : 
    <ul id="ul_85FC7FB19F7F4ADF83ABCA6DDB44CE19"> 
     ...
     <li id="li_73A8C5E905C54E2BB531A1FCDBC6AA1A"><b>qe</b>: Erreur  du (délai d’expiration, serveur inactif, etc.) </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 6 </td> 
   <td colname="col2"> de dépassement de délai </td> 
...
Si vous avez déjà suivi les campagnes publicitaires avec Adobe Analytics avant l’intégration DFA, il est possible que toutes les campagnes (courriel, recherche ou bannière) utilisent le même paramètre de chaîne de  pour identifier l’identifiant de campagne référente sur l’ du.
...
Pour activer l’intégration DFA sur votre site Web, vous devez mettre à jour votre code de collecte de données JavaScript en ajoutant ce qui suit :
...
Le code de collecte se compose des composants suivants :

* Paramètres d’intégration DFA
...
Le bloc Paramètres DFA Integrate définit les variables requises par l’intégration DFA. Les valeurs de chacune de ces variables proviennent des sources suivantes :
...
**tEvar**: Variable de transfert. Les Data Connectors prérenseignent cette variable à l’aide du nom de variable Analytics que vous avez spécifié comme variable d’affichage publicitaire dans l’assistant d’intégration. Ne modifiez pas cette valeur sans une coordination attentive avec les services d’ingénierie ou d’ingénierie d’Adobe.
...
**timeoutEvent**: de dépassement de délai. Les Data Connectors prérenseignent cette variable à l’aide du nom de variable Analytics que vous avez spécifié comme variable d’événement de dépassement de délai dans l’assistant d’intégration.

**requestURL**: Hôte DFA distant à  pour les informations publicitaires. Ne modifiez pas cette valeur, sauf si Adobe vous l’a demandé.

**maxDelay**: Indique le temps d’attente, en millisecondes, d’une réponse du serveur DFA Floodlight par le code de collecte de données JavaScript. Adobe recommande de faire des essais avec cette valeur afin de rechercher la valeur optimale en fonction du trafic sur votre site. Par exemple, si vous augmentez cette valeur, davantage de données DFA sont généralement collectées, mais vous risquez de perdre des données de base sur le visiteur si celui-ci quitte le site durant la période du délai. La réduction de cette valeur réduit le risque de perte des données d’accès, mais peut réduire la quantité de données DFA envoyées avec les données d’accès Adobe.

**visitCookie**: Nom du cookie utilisé pour limiter les appels DFA à une fois par visite.

**clickThroughParam** : chaîne de requête, généralement incluse sur toutes les publicités, qui avertit le module Integrate qu’un clic vient d’avoir lieu. La présence de ce paramètre dans la chaîne de  du provoque la demande pour les serveurs DFA Floodlight, que le ait déjà été interrogé au cours des 30 dernières minutes.

**newRsidsProp**: (Facultatif) Associé à une variable de propriété Trafic inutilisée. L’intégration DFA collecte et stocke cette valeur dans le cookie de visite afin d’identifier les suites de rapports qui ont collecté des données pour un visiteur particulier. Cette propriété n’est nécessaire qu’avec les implémentations personnalisées avec les services Adobe Engineering.

**Modules externes requis pour l’intégration**

L’ajout de code de collecte comprend des modules complémentaires qui améliorent le fonctionnement de l’intégration DFA :

* Limite le  DFA à une fois par visite
* Procure une certaine flexibilité eu égard au nom de cookie. Bien que la plupart des organisations utilisent s_dfa, vous pouvez utiliser n’importe quel nom de cookie valide pour l’intégration DFA.
* Élimine les redirections superflues. Les données d’affichage publicitaire sont collectées en temps réel ; pour cette raison, les serveurs de collecte Adobe et les serveurs DFA peuvent éventuellement échanger des données sur chaque page vue. Le plug-in bloque ces échanges de données lorsque les informations ne sont pas nécessaires.
...
* Une demande au service `fls.doubleclick.net/json` de DFA. Ce service peut répondre différemment selon la version de DFA que vous utilisez. Avec l’intégration DFA version 1.5 :
   ...
   * Cette balise d’emplacement provoque une redirection vers [!DNL integrate.112.2o7.net/dfa_echo]. Ce service traduit les informations sur le publicitaire en chaîne codée JSON (JavaScript Object Notati on). Ces données sont renvoyées avec une réponse HTTP 200 OK.

* Avec l’intégration DFA version 2.0 (fonction AdServing avancée activée) :

   * [!DNL fls.doubleclick.net] répond directement avec une réponse 200 OK.

Dans les deux cas, une demande réussie engendre une demande aux serveurs de collecte de données Adobe qui contient le paramètre vX, où X est votre numéro d’eVar d’affichage publicitaire. Cette valeur de paramètre se présente comme suit : DFA-XXXX-XXXX- XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-XXXX. Cette chaîne contient des données sur le dernier clic et la dernière impression pour le actuel.
...
*`s.maxDelay`* ne se contente pas d’encapsuler le temps pris par les communications réseau pour contacter DFA ; il représente également les délais qui s’écoulent avant que le navigateur ne déclenche et n’évalue le code JavaScript sur lequel reposent ces communications. En effet, le module Integrate commence le minuteur *`s.maxDelay`* après avoir inséré l’élément HTML dans le modèle DOM qui extrait les données du serveur DFA Floodlight. Le temps pris par le navigateur pour réellement initier la demande HTTP d’après ce nouvel élément HTML varie en fonction des autres images ou des fichiers JavaScript qui se chargent simultanément, de la vitesse de l’ordinateur des visiteurs et des mises en œuvre de navigateur spécifiques. De plus, lorsque les données JSON sont récupérées à partir du serveur DFA Floodlight, le code JavaScript doit être évalué par le navigateur. Cette opération est à nouveau entièrement contrôlée par le navigateur et peut être retardée s’il existe de grandes quantités de code JavaScript s’exécutant simultanément ou de nombreuses requêtes JavaScript asynchrones.

Ainsi, *`s.maxDelay`* doit être défini en fonction de la complexité de la page d’entrée et du délai réseau avec DFA. Sur certains sites, une méthode possible pour réduire la complexité consiste à déclencher le code de collecte Adobe au début du chargement de la page, de sorte qu’il n’y ait plus de problèmes dans le navigateur au moment où le serveur Floodlight est demandé.
...
1. Exécutez le [!DNL Timeout Event Report] (rapport Événement de dépassement de délai) pour vérifier le nombre de dépassements de délai qui ont eu lieu. N’oubliez pas qu’un délai d’expiration n’est collecté qu’une seule fois par.

Maintenant que les chiffres sont en main, calculer

```
Timeout Percentage = [Step 3] / [Step 2] * 100
```

Le pourcentage de dépassement de délai prend actuellement en compte tous les visiteurs sur le site. Certains de ces visiteurs peuvent en fait n’avoir aucune connexion avec DFA ; le dépassement de délai est donc trompeur. Pour améliorer ce calcul, une autre analyse consiste à prendre en compte seulement les visiteurs uniques sur les pages avec le paramètre `clickThroughParam` défini (par exemple, `?CID=1`). Cela montrera plus de précision.
...
Les dépassements de délai devraient normalement diminuer quand vous passerez à la version 2.0 de l’intégration, en raison de l’élimination des redirections 302. Les premières conclusions avec les clients bêta ont montré une réduction constante des dépassements de délai et donc davantage de données DFA sont collectées
