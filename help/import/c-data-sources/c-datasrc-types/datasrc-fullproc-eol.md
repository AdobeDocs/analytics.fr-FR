...
Pendant plusieurs années, les sources de données à traitement complet vous ont permis d’envoyer des données au niveau de l’accès à Adobe Analytics. Ces données ont été traitées de la même manière que les données collectées via nos bibliothèques JavaScript et notre SDK d’applications mobiles. En 2020, Adobe a publié la [API Bulk Data Insertion](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md), qui exécute les mêmes fonctions que les sources de données à traitement complet, mais avec des fonctionnalités supplémentaires. Cette rubrique fournit des détails sur les fonctionnalités supplémentaires fournies par l’API Bulk Data Insertion et décrit les différences de formats de fichier.

À compter du 25 mars 2021, Adobe a empêché la création de nouvelles connexions de sources de données à traitement complet. Les connexions existantes ont été prises en charge jusqu’à ce que le service soit complètement abandonné le 31 janvier 2022. Outre notre documentation standard, nous vous proposons une présentation de la [étapes nécessaires pour envoyer des données via l’API Bulk Data Insertion](https://adobe.ly/aabdia).

## Pourquoi avons-nous mis fin à cette fonctionnalité ?

L’API Bulk Data Insertion (BDIA) fournit des fonctionnalités supplémentaires tout en couvrant tous les cas d’utilisation pris en charge par le traitement complet. Vous êtes mieux servi en utilisant l’API Bulk Data Insertion.
...
* L’insertion de données en bloc prend en charge plusieurs options pour les identifiants visiteur. Vous pouvez envoyer Analytics ID et Marketing Cloud ID (voir [Identity Service](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=fr) pour en savoir plus). De plus, vous pouvez utiliser votre propre ID en tant que [valeur de départ pour la génération d’un ECID](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md#customer-id-and-experience-cloud-visitor-id-seeds).
...
* L’insertion de données en bloc comporte d’autres champs obligatoires. Voir [documentation](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md) pour plus d’informations.
...
| events | events | [Chaîne d’événements](https://experienceleague.adobe.com/docs/analytics/implementation/vars/page-vars/events/event-serialization.html?lang=en#vars), formaté selon la même syntaxe que la variable s.events . Par exemple : scAdd,event1,event7 |
...
| timestamp | date | Utiliser le format de date ISO 8601 AAAA-MM-JJThh:mm:ss±décalage_UTC (par exemple, 2021-09-01T12:00:00-07:00 ) ou le format d’heure Unix (nombre de secondes écoulées depuis le 1er janvier 1970). |
| trackingServer | Non pris en charge | Ne peut être fourni que par le biais de l’en-tête de colonne. |
| transactionID | Non pris en charge | Valeur couramment utilisée pour associer des activités d’utilisateur multicanaux en vue de la génération de rapports. Pour plus d’informations, voir [Guide de l’utilisateur des sources de données](https://experienceleague.adobe.com/docs/analytics/import/data-sources/datasrc-home.html?lang=en#data-sources). |
