...
>title="Observateurs et observatrices simultanés de médias"
...
Le panneau **[!UICONTROL Observateurs et observatrices simultanés de médias]** permet d’analyser les observateurs et observatrices simultanés au fil du temps. Il fournit également des informations détaillées sur le pic d’accès simultanés et offre la possibilité d’effectuer une répartition et une compaison.

Vous pouvez analyser les observateurs et observatrices simultanés pour déterminer où se sont produits les pic d’accès simultanés ou les abandons, afin d’obtenir des informations précieuses sur la qualité du contenu et l’engagement des observateurs et observatrices. Et pour faciliter le dépannage ou la planification du volume ou de l’échelle.

Dans Analysis Workspace, la mesure des observateurs et observatrices simultanés représente le nombre de personnes uniques qui visualisent vos flux de médias à un moment donné, quel que soit le nombre de sessions.
...
Pour utiliser un panneau **[!UICONTROL Observateurs et observatrices de médias]**, procédez comme suit :

1. Créez un panneau **[!UICONTROL Observateurs et observatrices simultanés de médias]**. Pour plus d’informations sur la création d’un panneau, consultez [Créer un panneau](panels.md#create-a-panel).
...
Vous pouvez configurer le panneau Observateurs et observatrices simultanés de médias à l’aide des paramètres d’entrée suivants :
...
![Vue de la répartition de la série Observateurs et observatrices simultanés de médias présentant 7 dimensions, segments ou périodes sur 10.](assets/concurrent-viewers-series-breakdown.png)
...
À tout moment, sélectionnez ![Modifier le panneau Observateurs et observatrices simultanés de médias](/help/assets/icons/Edit.svg) pour modifier et recréer le panneau.

Si vous avez sélectionné la répartition de séries, une ligne du graphique linéaire et une synthèse des chiffres s’affichent pour chacune d’elles :

![Sortie des observateurs et observatrices simultanés de médias.](assets/concurrent-viewers-output.png)

### Source de données

La seule mesure qui peut être utilisée dans ce panneau est **[!UICONTROL Observateurs et observatrices simultanés]** :

| Mesure | Description |
|---|---|
| **[!UICONTROL Observateurs et observatrices simultanés]** | Nombre de personnes uniques qui visualisent vos flux de médias à un moment précis, quel que soit le nombre de sessions. |

Un tableau à structure libre n’est pas disponible dans cet affichage. Pour afficher la source de données, vous pouvez la télécharger à partir du menu contextuel de visualisation sous forme de graphique linéaire et sélectionner **[!UICONTROL Télécharger les données au format CSV]**.  Les répartitions de séries sont incluses.

![Les options de sortie des observateurs et observatrices simultanés avec l’option « Télécharger les données au format CSV » mise en surbrillance.](assets/concurrent-viewers-download-csv.png)

## Questions fréquentes

| Question | Réponse |
|---|---|
| Où se trouve le tableau à structure libre ? Comment puis-je voir la source de données ? | Le tableau à structure libre n’est pas disponible dans cet affichage. Vous pouvez télécharger la source de données à partir du menu contextuel du graphique linéaire et sélectionner **[!UICONTROL Télécharger les données au format CSV]**. |
| Pourquoi ma granularité a-t-elle changé ? | La visualisation est limitée à 1 440 lignes de données (par exemple, 24 heures à une granularité au niveau des minutes).  Si une combinaison de période et de granularité génère plus de 1 440 lignes, la granularité est automatiquement mise à jour pour s’adapter à la période complète.<br><br>Lorsque vous passez d’une période plus grande à une période plus petite, la granularité est mise à jour au niveau de détail le plus bas possible une fois la période modifiée. Pour afficher une granularité plus élevée, modifiez le panneau et recréez la visualisation. |
| Comment puis-je comparer les noms de vidéo, les filtres, les types de contenu, etc. ? | Pour les comparer dans une visualisation unique, faites glisser des filtres, des dimensions ou des éléments de dimension spécifiques dans le filtre de répartition des séries.<br><br>L’affichage est limité à 10 répartitions.  Pour en afficher plus de 10, vous devez utiliser plusieurs panneaux. |
...
| Que signifie le fait d’avoir des observateurs simultanés avec une granularité supérieure à la minute ? | Avec une granularité supérieure à une minute, les observateurs simultanés représentent la somme des observateurs simultanés uniques pour toutes les minutes de cette période. Par exemple, les observateurs et observatrices simultanés avec une granularité au niveau de l’heure représentent la somme des observateurs et observatrices simultanés uniques pour toutes les minutes de cette heure. |
| Le panneau Espace de travail affiche-t-il les mêmes informations que le rapport sur les observateurs simultanés ? | Non.  Dans Analysis Workspace, les observateurs et observatrices simultanés sont définis comme le nombre de personnes uniques qui visualisent votre flux de médias à un moment précis, quel que soit le nombre de sessions. Quel que soit le nombre de sessions.<br><br>Cette mesure est différente de celle des observateurs et observatrices simultanés dans la section Rapports, qui utilise les sessions simultanées actives. L’utilisation de personnes uniques entraîne la suppression des pics indésirables aux limites de l’affichage (où les sessions se terminent et commencent en même temps). |
