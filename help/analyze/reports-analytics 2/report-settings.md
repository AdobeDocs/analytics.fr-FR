...
**[!UICONTROL Analytics]** > **[!UICONTROL Composants]** > **[!UICONTROL Tous les composants]** > **[!UICONTROL Paramètres des rapports]**

| Élément | Description |
|--- |--- |
| **Paramètres généraux** |  |
| Inclure la section des mesures connexes | Les mesures connexes sont étroitement liées au rapport que vous êtes en train de consulter (par exemple, les cinq premières pages du rapport Pages vues). |
| Afficher la moyenne Internet dans la section des informations | Exprime la valeur moyenne d’une statistique donnée, d’après des milliers de sites web d’entreprise. Une fois activée, cette section apparaît comme une colonne séparée dans les sections Résumé du rapport et Détails du rapport. Cette fonction s’applique uniquement aux rapports Trafic dans le groupe Technologie, ainsi qu’aux rapports sur les moteurs de recherche, les langues et les domaines. |
| Afficher la structure de menu par défaut d’Adobe | Ignore les paramètres définis dans les Outils d’administration, où les administrateurs personnalisent les menus des rapports selon les préférences des utilisateurs, en rétablissant les paramètres par défaut des menus de rapports. |
| Forcer la largeur des colonnes lors de l’affichage des rapports | Cette option force le dimensionnement des largeurs de colonnes pour les rendre uniformes et plus esthétiques. Ce paramètre est utile lorsque plus de trois mesures sont affichées. |
| **Graphiques et tableaux** |  |
| Mettre en évidence les week-ends dans les graphiques d’établissement de tendance | Met verticalement en évidence les dates des week-ends sur les graphiques des rapports de tendances. Il est ainsi beaucoup plus facile d’évaluer les rapports de tendances. |
| Inclure les prévisions dans le graphique et le résumé | Estime la probabilité qu’une statistique particulière a de se produire à l’avenir. Lorsque cette option est activée, la prévision apparaît dans la section Résumé du rapport. |
| Inclure des événements calendrier dans les rapports | Surveille les performances du site par rapport à des événements particuliers. Lorsque cette fonction est activée, ces événements apparaissent dans vos rapports. |
| Utiliser des graphiques Flash | Active les graphiques Flash dans vos rapports. Les graphiques Flash fournissent des images plus nettes et plus interactives pour les rapports ; toutefois, ils ne permettent pas de copier ni d’enregistrer facilement ces images. Si vous souhaitez pouvoir copier ou enregistrer rapidement les images, désactivez cette option (les images seront au format .gif). Si vous désélectionnez cette option, le bouton Copier graphique n’apparaîtra pas dans la barre d’outils des rapports. |
| Afficher les données « Toutes les autres » dans les graphiques sélectionnés | Cette option affiche toutes les autres données au-dessous des cinq premières regroupées dans un objet unique https://marketing.adobe.com/resources/help/fr_FR/reference/reports_conversion.html |
...
| **Accélération** |  |
| Permettre à l’Accélérateur de rapports de visionner les rapports plus rapidement | Cette option active l’accélérateur de rapports, qui utilise un algorithme temporel pour mettre en cache les rapports récemment demandés et examine seulement les éléments uniques, ce qui se traduit par une remise encore plus rapide des rapports. En mettant en cache les rapports demandés pendant 15 minutes, l’accélérateur de rapports peut extraire presque instantanément ces rapports lors de demandes ultérieures. Ce paramètre est utile si vous naviguez entre plusieurs rapports, si vous imprimez des rapports ou si vous accédez fréquemment aux mêmes rapports. Lorsque cette option est désactivée, le système régénère les rapports chaque fois que vous en faites la demande. |
...
| Séparateur de milliers | Sélectionnez un séparateur pour chaque millier (décimal ou virgule). De nombreux pays utilisent un point pour séparer les milliers. (ce séparateur s’applique à tous les nombres du système et pas seulement aux devises). |
