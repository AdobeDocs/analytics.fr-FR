---
description: Découvrez comment actualiser les requêtes après les avoir modifiées ou leur avoir appliqué des filtres, ou comment afficher des données plus récentes.
title: Actualisation d’une requête
...
Actualisez les requêtes après les avoir mises à jour avec des modifications, des filtres ou si vous devez afficher des données plus récentes. Vous pouvez sélectionner plusieurs demandes dans une feuille de calcul et les actualiser toutes en même temps.

Si vous augmentez la période d’une requête, puis que vous actualisez la requête, Report Builder ajoute des cellules pour s’adapter au nombre de périodes ajouté.
