...
description: Questions fréquentes relatives à Activity Map.
...
Questions fréquentes relatives à Activity Map.
...
Les [éléments d’autorisation](/help/admin/admin-console/permissions/product-profile.md) requis pour Activity Map sont les suivants :
...
+++Comment Activity Map prend-il en charge les applications monopages ?

Toutes les quelques secondes, Activity Map analyse la page web à la recherche de modifications. Activity Map trouve du nouveau contenu sur la page sans avoir à le recharger, mais ce nouveau contenu est toujours attribué à la première valeur de dimension de page.
...
* Lorsqu’une interaction d’un utilisateur crée du contenu, tous les nouveaux éléments déterminés par AppMeasurement en tant que lien sont ajoutés au tableau [!UICONTROL Liens sur la page]. Activity Map envoie une nouvelle requête de données qui inclut ces nouveaux liens. Les nouveaux liens apparaissent dans le tableau [!UICONTROL Liens sur la page] lorsque la requête de données est renvoyée.

+++

+++Activity Map fournit-il des données sur les liens affichés mais non cliqués ?

Non, Adobe ne suit pas automatiquement les liens qui n’ont été consultés que.
...
Oui. Les [colonnes de flux de données](/help/export/analytics-data-feed/c-df-contents/datafeeds-reference.md) utilisées par Activity Map sont les suivantes :
...
La méthode de désactivation d’Activity Map dépend de votre type d’implémentation :
...
+++Quelle est la configuration requise pour utiliser le recouvrement Activity Map ?

Vous pouvez utiliser la dernière version de Chrome, Edge ou Firefox avec l’extension Activity Map.

+++

+++Que dois-je prendre en compte lorsque j’utilise Activity Map pour obtenir des informations d’identification personnelle ?

Tenez compte des scénarios suivants où des données d’identification personnelle peuvent être collectées à l’aide d’Activity Map :

* **Liens d’e-mail** : s’il est possible de cliquer sur une adresse e-mail pour ouvrir le client de messagerie de l’utilisateur, Activity Map peut collecter l’adresse e-mail sur laquelle l’utilisateur a cliqué.
* **Liens d’ID utilisateur** : une fois qu’un visiteur s’est connecté, Activity Map peut enregistrer tous les liens contenant son ID utilisateur.
...
* **Liens contenant des informations personnelles** : pour les sites Web de soins de santé, les liens peuvent contenir des informations personnelles. Si un visiteur clique sur ces liens, Activity Map collecte le texte du lien.

+++

+++Quelles données Activity Map suit-il par défaut ?
...
+++Quels sont les exemples de liens qu’Activity Map suit automatiquement ?
...
+++Quels sont les exemples de liens qu’Activity Map NE suit PAS automatiquement ?
...
Voici quelques exemples dans lesquels Activity Map n’effectue pas le suivi des clics :
