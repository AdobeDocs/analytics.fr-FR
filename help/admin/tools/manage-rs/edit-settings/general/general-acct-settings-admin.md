...
Voir ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [Configuration des paramètres généraux du compte](https://experienceleague.adobe.com/en/docs/analytics-learn/tutorials/administration/manage-report-suites/configuring-general-account-settings){target="_blank"} pour une vidéo de démonstration.
...
| Fuseau horaire | Détermine l’heure et la date associées aux données du rapport. Si le fuseau horaire sélectionné observe l’heure d’été, les données horaires sont automatiquement ajustées pour refléter cette situation. Le changement du fuseau horaire d’une suite de rapports active crée un pic ou un creux dans les données du rapport. Adobe recommande d’effectuer cette modification en dehors des heures de pointe afin de minimiser l’impact. |
...
| ID d’organisation | Identifiant associé à votre société CX Enterprise mise en service. Cet identifiant correspond à une chaîne de 24 caractères alphanumériques, suivie de @AdobeOrg (obligatoire). |
