...
>[!NOTE]
>
>Les données de la session avec horodatage sont conservées pendant 92 jours au maximum. Cela signifie qu’une visite/session sera « gardée ouverte » pendant 92 jours, tandis que tout accès supplémentaire, qui n’intervient pas 30 minutes après l’accès précédent (au cours de la période d’accès), peut toujours être inclus dans la même visite/session. Tout « ancien » accès reçu dans le désordre produira des résultats « inconnus », car un certain nombre de facteurs (segmentation, attribution, expiration, etc.) influencent le fait que ces accès soient inclus ou non dans le compte rendu des performances.
