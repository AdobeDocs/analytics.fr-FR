...
Si vous ne souhaitez plus qu’une suite de rapports soit disponible pour vous et vos utilisateurs, vous pouvez la masquer. Les raisons possibles de masquer une suite de rapports incluent des modifications d’implémentation ou la création par erreur d’une suite de rapports.

Le masquage d’une suite de rapports empêche son affichage dans le menu déroulant du sélecteur de suite de rapports, dans les outils d’administration ou ailleurs. Cela rend les données de la suite de rapports inaccessibles via les interfaces Adobe Analytics jusqu’à ce que la suite de rapports soit démasquée.
