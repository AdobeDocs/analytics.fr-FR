...
| Query String Parameter | Lecture seule | Valeur d’un paramètre de chaîne de requête spécifié dans l’URL actuelle. |
...
| Domaine racine référent | Lecture seule | Domaine de page de l’URL de référence, à l’exclusion des sous-domaines. |
...
| Identifiant d’achat | Lecture + écriture | Variable d’implémentation [`purchaseID`](/help/implement/vars/page-vars/purchaseid.md). |
| État | Lecture + écriture | La variable d’implémentation [`state`](/help/implement/vars/page-vars/state.md) est supprimée. |
...
| Hiérarchie 1-5 | Lecture + écriture | [Hiérarchie](/help/components/dimensions/hierarchy.md) dimensions. |
...
Toutes les [variables de données contextuelles](/help/implement/vars/page-vars/contextdata.md) que cette suite de rapports a vues dans les demandes d’image précédentes. Si les règles de traitement ne placent pas de données contextuelles dans une autre variable, ces données sont définitivement perdues. Voir [Copier une variable de données contextuelles dans eVar](processing-rules-examples/processing-rules-copy-context-data.md) et [Définir un événement à l’aide d’une variable de données contextuelles](processing-rules-examples/processing-rules-copy-context-data-event.md) pour consulter des exemples d’utilisation.
...
| Achats | En écriture seule | La mesure [ Passages en caisse ](/help/components/metrics/checkouts.md). |
...
| Événement 1-1 000 | En écriture seule | [ Événements personnalisés ](/help/components/metrics/custom-events.md). |
| Consultations de produit | En écriture seule | La mesure [Vues des produits](/help/components/metrics/product-views.md). |
