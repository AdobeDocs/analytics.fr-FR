...
| !Suites de rapports UICONTROL à inclure] | Remplace la suite de rapports pour un rapport planifié ou tous les petits rapports d’un tableau de bord. Bien que, sur le plan technique, le nombre d’entrées de suites de rapports distinctes ne soit pas limité, il est conseillé de ne pas en définir plus de 50. Le nombre de courriels pouvant être inclus est, lui aussi, illimité. |
| !Adresses de messagerie UICONTROL] | Liste, délimitée par des virgules, de toutes les adresses qui recevront le rapport avec la nouvelle suite de rapports.  Click **!UICONTROL Click to Edit]**to specify the Email addresses to receive. Séparez les différentes adresses électroniques à l’aide d’un point-virgule (;). Press`<Enter>`when finished entering Email addresses.<br>Le champ Nombre d’e-mails affiche le nombre d’adresses électroniques associées actuellement à l’entrée de la suite de rapports. |
| !UICONTROL Dupliqué] | Crée une copie de la liste de publication. |
