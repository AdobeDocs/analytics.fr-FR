...
Accédez à la page  de réussite dans les paramètres de la suite de rapports :
...
| Enregistrement d’événement unique | **Enregistrer une fois par visite**: Lie le  donné à la session . Les comptes suivants d’un donné au cours d’une même visite sont ignorés. Ce type de sérialisation des  de ne nécessite aucune modification de l’implémentation.<br>**Utiliser l&#39;ID **de  : Lie le  donné à un ID personnalisé. Les comptes suivants d’un donné avec le même ID de  sont ignorés. Ce type de sérialisation  nécessite un ID personnalisé dans les accès pour dédupliquer les valeurs. See[Event ID serialization](../../../implement/vars/page-vars/events/event-serialization.md)in the Implement user guide. |
