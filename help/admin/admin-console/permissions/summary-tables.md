...
| Événements de succès | x |  |  |  | Accorde l’accès à Conversion > Événement de succès dans le Gestionnaire de suites de rapports. |
| Variables de trafic | x |  |  |  | Accorde l’accès à Trafic > Variables de trafic dans le Gestionnaire de suites de rapports. |
| Visiteur unique | x |  |  |  | Accorde l’accès à Conversion > Variable de visiteur unique. Non généralement utilisé dans les implémentations modernes. |
| Filtres URL | x |  |  |  | Accorde l’accès à Général > Filtres URL internes dans le Gestionnaire de suites de rapports. |
| Détection des anomalies |  | x |  |  | Accorde l’accès à Détection des anomalies dans Analysis Workspace. |
...
| Masquage des suites de rapports | x |  |  |  | Accorde l’accès à Admin > Paramètres de la société > Masquer les suites de rapports. Permet de masquer une suite de rapports dans l’entreprise, quel que soit son accès. |
| Intégrations (créer) | x |  |  |  | Accorde l’accès à Admin > Data Connectors et permet à l’utilisateur de créer des Data Connectors. |
| Intégrations (supprimer) | x |  |  |  | Accorde l’accès à Admin > Data Connectors et permet à l’utilisateur de supprimer des Data Connectors. |
| Intégrations (mettre à jour) | x |  |  |  | Accorde l’accès à Admin > Data Connectors et permet à l’utilisateur de modifier les configurations de connecteur existantes. |
| Journaux | x |  |  |  | Accorde l’accès à Admin > Journaux. |
| Logs – Web Services | x |  |  |  | Accorde l’accès aux journaux d’extraction par le biais de l’API 1.4. |
| Actions en attente | x |  |  |  | Accorde l’accès à Admin > Paramètres de la société > Actions en attente. |
| Sécurité | x |  |  |  | Accorde l’accès à Admin > Paramètres de la société > Gestionnaire de sécurité. |
| Utilisation des appels au serveur | x |  |  |  | Accorde l’accès à Admin > Utilisation des appels serveur. |
| Support technique | x |  |  |  | Accorde l’accès à Admin > Paramètres de la société > Informations sur l’assistance. |
...
| Accès aux services Web |  | x |  |  | Permet d’utiliser l’API, notamment l’authentification auprès de sites tiers et l’envoi d’appels d’API. |
...
| Alliance de marques |  |  |  | x | N’est plus utilisé. Accorde l’accès à Admin > Paramètres de la société > Alliance de marques. |
| Utilisateurs sous licence Excel |  |  |  | x | Inutilisé. |
| Administration des applications mobiles |  |  |  | x | Inutilisé. |
| Gestion des autorisations |  |  |  | x | N’est plus utilisé. Accorde l’accès à l’interface de gestion des utilisateurs héritée sous Admin > Gestion des utilisateurs. |
