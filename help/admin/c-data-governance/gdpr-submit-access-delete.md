...
Chaque fichier combine les données de toutes vos suites de rapports, supprimant automatiquement les copies supplémentaires des accès répliqués. Vous pouvez décider parmi ces fichiers lequel renvoyer au sujet des données. Vous pouvez également extraire certaines de ces données et les combiner avec d&#39;autres données d&#39;autres systèmes avant de les renvoyer à la personne concernée.
...
* La première requête est une requête d’accès utilisant un ID de cookie Adobe Analytics traditionnel (AAID).
* La seconde requête est également une requête d’accès, mais utilise un cookie MCID/ECID.
* La troisième requête demande à la fois l’accès et la suppression pour les ID spécifiés. Bien que l’extension d’ID soit spécifiée pour toutes les demandes, elle concerna principalement cette troisième demande, puisque c’est la seule qui utilise des ID non liés à un cookie. Par conséquent, cette requête détecte également les identifiants de cookie associés à tout périphérique avec l’ID de gestion de la relation client ou l’adresse électronique spécifiée, et développe la requête pour inclure également ces identifiants.
...
Cette section contient des détails sur l’accès et la suppression des réponses.

**Détails de la réponse d’accès**
...
* Fichiers de personne : dérivés des accès contenant une étiquette ID-PERSON correspondante
   ...
   * Un fichier .CSV avec une ligne pour chaque accès correspondant et une colonne pour chaque champ avec une étiquette ACC-ALL ou ACC-PERSON, trié par horodatage.
...
* Fichiers d’appareil : dérivés des accès où l’un des champs correspondait à un ID-DEVICE spécifié, mais pas à un ID-PERSON spécifié
   ...
   * Un fichier .CSV avec une ligne pour chaque accès correspondant et une colonne pour chaque champ avec une étiquette ACC-ALL, triée par horodatage.
   * Fichier récapitulatif en HTML avec une entrée pour chaque étiquette ACC-ALL. Chaque entrée  toutes les valeurs uniques de ce champ et le nombre de fois où elles se sont produites. Les champs contenant des horodatages sont arrondis afin de spécifier uniquement des jours uniques.
...
Vous pouvez décider parmi celles-ci laquelle renvoyer au sujet des données. Vous pouvez également extraire certaines de ces données et les combiner avec d&#39;autres données d&#39;autres systèmes avant de les renvoyer à la personne concernée.

**Supprimer les détails de la réponse**
