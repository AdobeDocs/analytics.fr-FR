---
description: 'Les ID que vous soumettez ne couvrent pas toujours toutes les données d’accès qu’Analytics peut associer au sujet des données. Analytics peut créer un ensemble étendu d’ID pour inclure ces données associées aux demandes en vertu du RGPD. Vous pouvez demander cette option avec un paramètre facultatif pour chaque requête GDPR que vous envoyez, ajoutée à la requête JSON. '
...
>
>Veuillez contacter votre gestionnaire de compte (CSM) pour coordonner l'analyse des informations d'identification personnelle ou des données avec notre équipe de conseillers Architectes Ingénieurs.
