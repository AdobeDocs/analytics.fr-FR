...
Adobe génère automatiquement un identifiant visiteur pour chaque visiteur. Cette valeur est identique à la valeur concaténée des colonnes `visid_high` et `visid_low` dans les flux de données. Voir [Référence des colonnes de données](../../export/analytics-data-feed/c-df-contents/datafeeds-reference.md) pour plus d’informations.
