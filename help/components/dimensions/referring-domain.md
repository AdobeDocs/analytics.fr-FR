...
La dimension « Domaine référent » indique les domaines sur lesquels les visiteurs effectuent un clic pour accéder à votre site. Cette dimension est utile pour identifier les sites tiers qui génèrent le plus de trafic sur le vôtre. Un lien doit exister sur le site externe et un visiteur doit cliquer dessus pour que l’élément de dimension s’affiche.
...
Le même rapport peut montrer des résultats différents entre Analysis Workspace et Data Warehouse. analysis workspace rapporte le domaine référent pour chaque page, à l’exception des valeurs qui correspondent aux filtres d’URL internes. Le Data Warehouse signale uniquement le premier domaine référent de la visite et ignore les filtres d’URL internes.
...
Les éléments de Dimension incluent les domaines sur lesquels les visiteurs cliquent pour accéder à votre site. If a hit does not have any referrer data (either set or persisted), it groups under the dimension item `"Typed/Bookmarked"`. Cet élément de dimension signifie qu’il n’y avait aucune valeur de parrain, par exemple si le visiteur a saisi manuellement l’adresse du navigateur dans la barre d’adresse ou a cliqué sur un signet. L’élément `"Typed/Bookmarked"` de dimension s’affiche également pour les redirections qui ne s’adaptent pas à Analytics. Voir [Redirections et alias](/help/technotes/redirects.md) dans le guide de l’utilisateur Technotes.
