...
La publication d’un segment Adobe Analytics dans Experience Cloud vous permet d’utiliser ce segment pour le marketing   dans [!DNL Audience Manager] et dans d’autres  de, y compris Adobe [!DNL Advertising Cloud], [!DNL Target] et [!DNL Campaign]. Les mises à jour récentes ont considérablement optimisé le processus de publication. Vous pouvez désormais publier des segments Analytics dans Experience Cloud en moins de 8 heures. Utilisez ces segments pour activer   de dans le Gestionnaire de de  vers toutes les destinations en aval.

Nous avons également porté à 75 le nombre maximum de segments Adobe Analytics publiables (contre 20). You can view published segments in [!UICONTROL Analytics > Components > Segments].
...
* **Limites** d&#39;adhésion :   partagées avec le groupe depuis Adobe Analytics ne peut pas dépasser 20 millions de membres uniques. [!DNL Experience Cloud]
...
>[!NOTE]
>Une fois par semaine, toutes les données seront entièrement synchronisées pour tenir compte des écarts ou des incohérences qui n’ont pas été capturés au cours de la semaine précédente.
...
1. Cochez la case **[!UICONTROL Publier ce segment dans Experience Cloud (pour *suite de rapports*)]**.
...
1. Téléchargez et installez [Adobe Experience Cloud Debugger](https://docs.adobe.com/content/help/fr-FR/analytics/implementation/testing-and-validation/debugger.html) dans Chrome Web Store.
...
Demdex ([adobe.demdex.net](https://docs.adobe.com/content/help/fr-FR/audience-manager/user-guide/reference/demdex-calls.translate.html) dans l’exemple ci-dessous). Le champ Demdex est l’UUID AAM défini sur le
...
La case [!UICONTROL Publier ce segment dans Experience Cloud], qui s’affiche pendant le processus de création d’un segment dans Adobe Analytics, rend le segment disponible dans la bibliothèque d’audiences personnalisées d’Adobe Target. Un segment créé dans Analytics ou dans Audience Manager peut être utilisé pour des activités dans Target. Vous pouvez par exemple créer des activités de campagne d’après les mesures de conversion d’Analytics et les segments d’audience créés dans Analytics.
