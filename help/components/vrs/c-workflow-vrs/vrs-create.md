...
* Dans le gestionnaire des suites de rapports virtuelles, vous ne pouvez afficher que vos propres suites de rapports virtuelles. Vous devez cliquer sur « Tout afficher » pour afficher les suites de rapports virtuelles des autres utilisateurs.
...
| **Paramètres de visite des applications mobiles** | Permet de modifier la manière dont les visites sont définies pour les accès aux applications mobiles collectés par les SDK mobiles d’Adobe. Ces paramètres sont non destructifs et s’appliquent uniquement à Analysis Workspace. |
...
1. Faites glisser les composants (dimensions, mesures, segments ou plages de dates) que vous souhaitez inclure dans la suite de rapports virtuelle dans la [!UICONTROL Composants inclus] .

1. Une fois que vous avez terminé, cliquez sur **[!UICONTROL Enregistrer]**.

## Aperçu des données

Sur le côté droit de chaque onglet, vous pouvez prévisualiser le total des accès, le total des visites et le total des visiteurs dans cette suite de rapports virtuelle, par rapport à la suite de rapports d’origine.
