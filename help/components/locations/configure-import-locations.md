...
# Configurer des emplacements d’export et d’import cloud
...
1. Pour remplir le formulaire de configuration de l’emplacement, continuez avec [Configurer un emplacement](#configure-a-location). Utilisez les informations de la section correspondant au type de compte que vous avez sélectionné dans le champ [!UICONTROL **Comptes d’emplacement**]. (Des types de compte hérités supplémentaires sont également disponibles, mais ne sont pas recommandés.)
...
Les données de flux de données peuvent être distribuées vers un emplacement FTP hébergé par Adobe ou par le client ou la cliente. Spécifiez le répertoire Utilisez le champ de chemin d’accès pour placer les fichiers de flux dans un dossier.
