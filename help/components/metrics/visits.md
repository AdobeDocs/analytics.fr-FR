...
* Efface ses cookies en cours de session et continue à parcourir votre site.
...
* Pour les suites de rapports standard, contactez l’assistance clientèle pour demander que le délai de visite (temps écoulé entre les accès qui entraîne le démarrage d’une nouvelle visite) soit raccourci pour une suite de rapports donnée. Le délai de visite des suites de rapports standard ne peut pas dépasser 30 minutes. Par conséquent, vous pouvez seulement le raccourcir.
